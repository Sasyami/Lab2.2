...
class Polynomial{
private:
    Sequence<T>* coefficients; //Если что поменять на сиквенс
    size_t size;
public:
    Polynomial(){
    ...
    Polynomial(T* items,size_t size1){
        coefficients = new ArraySequence(items,size1);
        size = size1;
    }
    ~Polynomial(){
        delete coefficients;
...
public:
    ...
    Polynomial<T> operator- (const Polynomial<T>& poly){
        ...
        if (poly.size>size) {
            ...
            for (int i =size;i<poly.size;i++){
                new_pol.coefficients->Set(poly.coefficients->Get(i) * (-1), i);
    ...
    Polynomial<T>& operator= (const Polynomial<T>& poly) { //here, set problem
